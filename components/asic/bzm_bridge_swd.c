...
#include "rp2040_flash_stub_bin.h"
...
#define SWD_STUB_STATUS_IDLE BZM_BRIDGE_STUB_STATUS_IDLE
#define SWD_STUB_STATUS_BUSY BZM_BRIDGE_STUB_STATUS_BUSY
#define SWD_STUB_STATUS_OK BZM_BRIDGE_STUB_STATUS_OK
...
#define SWD_STUB_STAGE_IDLE BZM_BRIDGE_STUB_STAGE_IDLE
#define SWD_STUB_STAGE_ERASE 1u
#define SWD_STUB_STAGE_PROGRAM 2u
#define SWD_STUB_STAGE_DONE BZM_BRIDGE_STUB_STAGE_DONE
...
bzm_bridge_stub_poll_result_t bzm_bridge_flash_stub_poll_result(bool observed_running, bool halted, uint32_t status, uint32_t stage)
{
    if (!halted)
        return BZM_BRIDGE_STUB_POLL_WAIT;
    if (status == SWD_STUB_STATUS_OK && stage == SWD_STUB_STAGE_DONE) {
        return BZM_BRIDGE_STUB_POLL_COMPLETE;
    }
    if (!observed_running && status == SWD_STUB_STATUS_IDLE) {
        return BZM_BRIDGE_STUB_POLL_WAIT;
    }
    return BZM_BRIDGE_STUB_POLL_FAILED;
}
...
static esp_err_t swd_halt_attached_core(uint32_t * dhcsr_out)
{
    ...
    ESP_RETURN_ON_ERROR(swd_mem_write_word32(SWD_DHCSR_ADDR, SWD_DHCSR_DBGKEY | SWD_DHCSR_C_DEBUGEN | SWD_DHCSR_C_HALT), TAG,
                        "dhcsr halt write failed");
...
static esp_err_t swd_run_stub_attached(uint32_t ctx_addr, uint32_t * stub_status_out)
{
    ...
    for (unsigned int i = 0; i < SWD_RUN_POLL_LIMIT; i++) {
        ...
        if ((dhcsr & SWD_DHCSR_S_HALT) != 0) {
            ESP_RETURN_ON_ERROR(swd_mem_read_word32(ctx_addr + offsetof(swd_flash_stub_ctx_t, status), &stub_status), TAG,
                                "read stub status failed");
            (void) swd_mem_read_word32(ctx_addr + offsetof(swd_flash_stub_ctx_t, stage), &stub_stage);
            (void) swd_mem_read_word32(ctx_addr + offsetof(swd_flash_stub_ctx_t, progress), &stub_progress);
            bzm_bridge_stub_poll_result_t poll_result = bzm_bridge_flash_stub_poll_result(ran, true, stub_status, stub_stage);
            if (poll_result == BZM_BRIDGE_STUB_POLL_WAIT) {
                if ((i & 0x1Fu) == 0x1Fu) {
                    swd_cooperate();
                }
                continue;
            ...
            if (poll_result == BZM_BRIDGE_STUB_POLL_COMPLETE && total != 0 && last_percent < 100) {
                ESP_LOGI(TAG, "rpflash: erase+program 100%%");
            }
            return poll_result == BZM_BRIDGE_STUB_POLL_COMPLETE ? ESP_OK : ESP_FAIL;
        }
    }
    ...
    (void) swd_mem_read_word32(ctx_addr + offsetof(swd_flash_stub_ctx_t, progress), &stub_progress);
    ESP_LOGE(TAG,
             "stub timeout: ran=%s dhcsr=0x%08" PRIX32 " pc=0x%08" PRIX32 " r0=0x%08" PRIX32 " ctx.status=0x%08" PRIX32
             " ctx.stage=0x%08" PRIX32 " ctx.progress=%" PRIu32,
...
static esp_err_t swd_configure_pins(void)
{
...
static esp_err_t swd_halt_core(uint32_t targetsel)
{
    uint32_t dhcsr = 0;
    esp_err_t err;
    ...
    if (err == ESP_OK && (dhcsr & SWD_DHCSR_S_HALT) == 0) {
        return ESP_ERR_INVALID_STATE;
    }
    return err;
}

static esp_err_t swd_run_flash_operation(uint32_t op, uint32_t flash_offset, uint32_t count, uint32_t data_address)
{
    swd_flash_stub_ctx_t ctx = {
        .op = op,
        .status = SWD_STUB_STATUS_IDLE,
        .stage = SWD_STUB_STAGE_IDLE,
        .progress = 0,
    ...
    ESP_RETURN_ON_ERROR(swd_mem_write_block(SWD_RP2040_STUB_CTX_ADDR, (const uint8_t *) &ctx, sizeof(ctx)), TAG,
                        "stub context upload failed");
    ESP_RETURN_ON_ERROR(swd_run_stub_attached(SWD_RP2040_STUB_CTX_ADDR, &stub_status), TAG, "stub operation failed");
    return stub_status == SWD_STUB_STATUS_OK ? ESP_OK : ESP_FAIL;
}

static void report_progress(bzm_bridge_flash_progress_fn progress, void * context, bzm_bridge_flash_phase_t phase, size_t completed,
                            size_t total)
{
    if (progress != NULL) {
        progress(context, phase, completed, total);
    }
}

bool bzm_bridge_flash_next_chunk(size_t image_size, size_t offset, bzm_bridge_flash_chunk_t * chunk)
{
    if (chunk == NULL || image_size == 0 || offset >= image_size || offset % BZM_BRIDGE_FLASH_PAGE_SIZE != 0) {
        return false;
    }

    size_t remaining = image_size - offset;
    size_t source_length = remaining < BZM_BRIDGE_FLASH_CHUNK_SIZE ? remaining : BZM_BRIDGE_FLASH_CHUNK_SIZE;

    if (source_length >= BZM_BRIDGE_FLASH_PAGE_SIZE) {
        source_length -= source_length % BZM_BRIDGE_FLASH_PAGE_SIZE;
    }

    chunk->source_length = source_length;
    chunk->program_length = source_length < BZM_BRIDGE_FLASH_PAGE_SIZE ? BZM_BRIDGE_FLASH_PAGE_SIZE : source_length;
    return chunk->source_length != 0;
}

esp_err_t bzm_bridge_swd_flash(const uint8_t * image, size_t image_size, bzm_bridge_flash_progress_fn progress,
                               void * progress_context)
{
    if (image == NULL || image_size == 0 || image_size > BZM_BRIDGE_FLASH_CAPACITY) {
        return ESP_ERR_INVALID_ARG;
    }

    const size_t erase_length = (image_size + SWD_FLASH_SECTOR_SIZE - 1u) & ~(SWD_FLASH_SECTOR_SIZE - 1u);
    uint8_t tail[BZM_BRIDGE_FLASH_PAGE_SIZE];
    esp_err_t err;

    report_progress(progress, progress_context, BZM_BRIDGE_FLASH_PREPARING, 0, image_size);
    ESP_RETURN_ON_ERROR(swd_configure_pins(), TAG, "SWD pin configuration failed");

    ESP_LOGI(TAG, "halting RP2040 core1");
    err = swd_halt_core(RP2040_TARGETSEL_CORE1);
    if (err != ESP_OK) {
        return err;
    }
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "uploading %u-byte SRAM flash stub", (unsigned) RP2040_FLASH_STUB_BIN_SIZE);
        err = swd_mem_write_block(SWD_RP2040_STUB_BASE, RP2040_FLASH_STUB_BIN, RP2040_FLASH_STUB_BIN_SIZE);
    }

    if (err == ESP_OK) {
        report_progress(progress, progress_context, BZM_BRIDGE_FLASH_ERASING, 0, image_size);
        err = swd_run_flash_operation(SWD_STUB_OP_ERASE, 0, (uint32_t) erase_length, 0);
    }

    size_t offset = 0;
    report_progress(progress, progress_context, BZM_BRIDGE_FLASH_PROGRAMMING, 0, image_size);
    while (err == ESP_OK && offset < image_size) {
        bzm_bridge_flash_chunk_t chunk;
        if (!bzm_bridge_flash_next_chunk(image_size, offset, &chunk)) {
            err = ESP_ERR_INVALID_SIZE;
            break;
        }

        const uint8_t * source = image + offset;
        if (chunk.program_length != chunk.source_length) {
            memset(tail, 0xff, sizeof(tail));
            memcpy(tail, source, chunk.source_length);
            source = tail;
        }

        err = swd_mem_write_block(SWD_RP2040_STUB_DATA_BASE, source, chunk.program_length);
        if (err == ESP_OK) {
            err = swd_run_flash_operation(SWD_STUB_OP_PROGRAM, (uint32_t) offset, (uint32_t) chunk.program_length,
                                          SWD_RP2040_STUB_DATA_BASE);
        }
        if (err == ESP_OK) {
            offset += chunk.source_length;
            report_progress(progress, progress_context, BZM_BRIDGE_FLASH_PROGRAMMING, offset, image_size);
        }
    }

    offset = 0;
    report_progress(progress, progress_context, BZM_BRIDGE_FLASH_VERIFYING, 0, image_size);
    while (err == ESP_OK && offset < image_size) {
        size_t length = image_size - offset;
        if (length > BZM_BRIDGE_FLASH_CHUNK_SIZE) {
            length = BZM_BRIDGE_FLASH_CHUNK_SIZE;
        }
        err = swd_mem_verify_block(SWD_RP2040_XIP_BASE + (uint32_t) offset, image + offset, length);
        if (err == ESP_OK) {
            offset += length;
            report_progress(progress, progress_context, BZM_BRIDGE_FLASH_VERIFYING, offset, image_size);
        }
    }

    if (err == ESP_OK) {
        report_progress(progress, progress_context, BZM_BRIDGE_FLASH_RESETTING, image_size, image_size);
        err = swd_reset_system_attached();
    }
