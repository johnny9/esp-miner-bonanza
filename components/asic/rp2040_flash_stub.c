...
static inline void *rom_func_lookup_inline(uint32_t code)
{
    ...
    uint16_t *table = (uint16_t *)rom_hword_as_ptr(0x14);
...
void rp2040_flash_stub_entry(flash_stub_ctx_t *ctx)
{
    ...
    rom_flash_exit_xip_fn flash_exit_xip =
        (rom_flash_exit_xip_fn)rom_func_lookup_inline(
    ...
    if (!connect_internal_flash || !flash_exit_xip || !flash_range_erase ||
    ...
    if (ctx->op == FLASH_STUB_OP_ERASE) {
        ...
        flash_range_erase(ctx->flash_offset, ctx->count,
                          FLASH_BLOCK_SIZE, FLASH_BLOCK_ERASE_CMD);
    } else if (ctx->op == FLASH_STUB_OP_PROGRAM) {
        ctx->stage = FLASH_STUB_STAGE_PROGRAM;
        flash_range_program(ctx->flash_offset,
                            (const uint8_t *)(uintptr_t)ctx->data_addr,
                            ctx->count);
